...
def uuid() -> str:
    '''
    This method creates an universal UUID which is representing a 128-bit number. Specifications
    ...
    def hexoctets(amount: int) -> str:
        if amount == 2:
            return '%s%s' % (hexoctet(), hexoctet())
        elif amount == 4:
            return '%s%s%s%s' % (hexoctet(), hexoctet(), hexoctet(), hexoctet())
        elif amount == 6:
            return '%s%s%s%s%s%s' % (hexoctet(), hexoctet(), hexoctet(), 
            hexoctet(), hexoctet(), hexoctet())
        else:
            return None

    return '%s-%s-%s-%s-%s' % (hexoctets(4), hexoctets(2), hexoctets(2), hexoctets(2), hexoctets(6))
